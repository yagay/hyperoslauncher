...
final class HyosProbeMatrix {
    ...
    static String collect() {
        StringBuilder out=new StringBuilder("DesktopGridX LSPosed 7869 / HYOS dual-registration matrix\n");
        append(out,"=== A. module contract ===",RootShell.run(
                ...
                "echo '-- legacy xposed_init --'; unzip -p \"$APK\" assets/xposed_init 2>/dev/null; echo; " +
                "echo '-- legacy native_init --'; unzip -p \"$APK\" assets/native_init 2>/dev/null; echo; " +
        ...
        append(out,"=== B. LSPosed registration / scope ===",RootShell.run(
                ...
                "echo '-- parsed LSPosed scope cache --'; for F in /data/adb/lspd/log/scopes.txt /data/adb/lsposed/log/scopes.txt /data/adb/lspd/logs/scopes.txt /data/adb/lsposed/logs/scopes.txt; do [ -f \"$F\" ] && grep -n -A30 -B3 -E 'com\\.miui\\.home|com\\.yagay\\.desktopgridx|HookEntry|LegacyHookEntry|libdesktopgridx_hyos' \"$F\" | tail -n 500; done",
                20,3*1024*1024));
        ...
        append(out,"=== D. staged execution evidence ===",RootShell.run(
                "LOG=$(logcat -d -b all -v threadtime -t 60000 2>/dev/null); " +
                ...
                "echo legacy_entry_seen=$(printf '%s\\n' \"$LOG\" | grep -c 'DGX_LEGACY_ENTRY' || true); " +
                ...
                "echo '-- DesktopGridX stages --'; printf '%s\\n' \"$LOG\" | grep -E 'DGX_HYOS_CTOR|DGX_MODERN_ENTRY|DGX_LEGACY_ENTRY|DGX_NATIVE_ENTRY|libapp_launcher callback install|LSPosed hook OK|LSPosed hook failed' | tail -n 4000 || true; " +
        ...
        append(out,"=== F. LSPosed / HYOS dispatch timeline ===",RootShell.run(
                "logcat -d -b all -v threadtime -t 60000 2>/dev/null | grep -iE 'DesktopGridX|DockSink|DGX_LEGACY_ENTRY|native_api:|Loading module native library|libdesktopgridx_hyos|native_init|hyos_spawner|HyperOS Runtime|HyperOS Rust Runtime|LSPosed|libapp_launcher' | tail -n 14000 || true",
                35,7*1024*1024));

        append(out,"=== G. linker / SELinux / crash ===",RootShell.run(
                "echo '-- linker --'; logcat -d -b all -v threadtime -t 60000 2>/dev/null | grep -iE 'desktopgridx_hyos|cannot locate|needed by|dlopen|linker.*error|namespace' | tail -n 4000 || true; " +
